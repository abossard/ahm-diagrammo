#!/usr/bin/env node
// swimlane-auto.mjs — auto-layout swimlane SVGs for EVERY health-model diagram in the article.
// Parses each ```mermaid flowchart BT``` block into a graph, layers it (longest path to the root,
// with signal nodes pinned to the bottom lane), orders columns by barycenter to reduce crossings,
// and renders a technical-documentation figure: horizontal lanes, per-entity icons, signal cards,
// roll-up edges colored by child health state, dashed propagation edges with labels, and a legend.
...
// ---------- palette ----------
const INK = "#242424", MUTED = "#605e5c", LANE_LABEL = "#323130", HAIR = "#e6e4e2";
const BG = "#ffffff", BAND = "#faf9f8", AZURE = "#0078D4";
const STATE = {
...
// ---------- icons (24x24, stroke) ----------
function icon(name, stroke = MUTED) {
...
// relationship label as a rounded pill sitting on the connector (white fill, thin border, bold text)
function edgeLabelPill(label, cx, cy) {
  const fs = 10.5, padX = 10, h = 20, tw = label.length * fs * 0.56, w = tw + padX * 2;
  return `<g stroke="none">`
    + `<rect x="${(cx - w / 2).toFixed(1)}" y="${(cy - h / 2).toFixed(1)}" width="${w.toFixed(1)}" height="${h}" rx="${h / 2}" fill="#ffffff" stroke="#d8d6d4" stroke-width="1"/>`
    + `<text x="${cx.toFixed(1)}" y="${(cy + fs * 0.34).toFixed(1)}" font-size="${fs}" font-weight="600" fill="${INK}" text-anchor="middle">${esc(label)}</text>`
    + `</g>`;
}
// small metric bar-chart glyph for signal-table rows (product "Azure resource metric" style)
function metricIcon(x, y, size = 14) {
  const s = size / 16;
  return `<g transform="translate(${x},${y}) scale(${s})">
    <rect x="1.5" y="8" width="2.6" height="6" rx="0.6" fill="#8661c5"/>
    <rect x="5.7" y="4.5" width="2.6" height="9.5" rx="0.6" fill="#0078D4"/>
    <rect x="9.9" y="6.5" width="2.6" height="7.5" rx="0.6" fill="#3fb0ac"/>
  </g>`;
}
// status symbol: healthy = green disc + white check; others = filled disc in state color
function statusDot(state, cx, cy) {
  const st = STATE[state] || STATE.unknown;
  if (state === "healthy") {
    return `<circle cx="${cx}" cy="${cy}" r="6" fill="${st.dot}"/><path d="M${cx - 2.7} ${cy} l1.9 1.9 l3.4 -3.9" fill="none" stroke="#fff" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>`;
  }
...
// ---------- extract mermaid blocks from the article ----------
function slugify(s) { return s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60); }
function extractBlocks(md) {
  const lines = md.split("\n");
  const blocks = [];
  let heading = "diagram", inBlock = false, buf = [], used = new Map();
  for (const line of lines) {
    ...
    if (inBlock && line.trim() === "```") {
      inBlock = false;
      let base = slugify(heading) || "diagram";
      const n = (used.get(base) || 0) + 1; used.set(base, n);
      blocks.push({ slug: n === 1 ? base : `${base}-${n}`, heading, code: buf.join("\n") });
      continue;
    }
...
// single entity it points to, attach its metric lines as signal rows, then drop the node and edge.
const SIGNAL_WORDS = new Set(["signal", "signals"]);
function foldSignals(g) {
  const remove = new Set();
  const isSig = (id) => g.nodes.get(id)?.state === "signal";
  // map signal node -> its targets (entities it points to)
  ...
  for (const [sigId, targets] of targetsOf) {
    ...
    for (const t of owners) {
      ...
      for (const m of metrics) owner.signals.push({ name: m, state: "healthy" });
    }
    remove.add(sigId);
  }
  // any signal node with no outgoing edge stays as a compact node (rare); only remove folded ones
  g.edges = g.edges.filter((e) => !remove.has(e.from) && !remove.has(e.to));
  for (const id of remove) g.nodes.delete(id);

  // synthesize per-row state (option A): a non-healthy entity's primary signal takes that state
  for (const n of g.nodes.values()) {
    if (!n.signals || !n.signals.length) continue;
    if (n.state === "degraded" || n.state === "unhealthy") n.signals[0].state = n.state;
    // synthesize a plausible, deterministic Result value per row
    for (const row of n.signals) row.result = synthResult(row.name, row.state);
  }
  return g;
}
...
function layout(g) {
  ...
  const maxD = Math.max(...ids.map((i) => depth.get(i)), 0);
...
function render(g, lay, title) {
  const { laneNodes, L } = lay;
  const GAP = 30, HEADER_H = 34, ROW_H = 18, TABLE_PAD = 8, COMPACT_H = 58;
  // signal-bearing cards are wider (they hold a 4-column table)
  ...
  // Pre-pass: every labeled edge (solid OR dashed) gets a pill. Group by the parent's lane; within a
  // lane, greedily assign vertical "levels" so pills whose x-ranges overlap land on different levels
  // (guarantees no pill overlap). The number of levels in a lane's gap sets how much extra vertical
  // room that lane reserves below its cards.
  ...
  for (const [ln, es] of labByPLane) {
    // interval = the edge's horizontal span (child x .. parent x), padded, so overlapping LINES (not
    // just pills) get separate levels. This kills muddy same-level line overlaps near a shared parent.
  ...
  out.push(`<defs><filter id="cs" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="0" dy="1" stdDeviation="1.3" flood-color="#000" flood-opacity="0.10"/></filter>`);
  out.push(Object.entries(STATE).map(([k, v]) => `<marker id="ar-${k}" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M1 1 L9 5 L1 9" fill="none" stroke="${v.border}" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></marker>`).join(""));
  out.push(`</defs><rect width="${W}" height="${H}" fill="${BG}"/>`);

  // lane bands + labels
  laneY.forEach((l, i) => {
    out.push(`<rect x="0" y="${l.top}" width="${W}" height="${l.h}" fill="${i % 2 ? BAND : BG}"/>`);
    out.push(`<line x1="0" y1="${l.top}" x2="${W}" y2="${l.top}" stroke="${HAIR}"/>`);
    const lx = M.left + contentW + 24, lines = wrapLabel(labels[i] || "");
    if (lines.length === 1) out.push(`<text x="${lx}" y="${l.mid + 5}" font-size="13" font-weight="700" fill="${LANE_LABEL}">${esc(lines[0])}</text>`);
    else { out.push(`<text x="${lx}" y="${l.mid - 2}" font-size="13" font-weight="700" fill="${LANE_LABEL}">${esc(lines[0])}</text>`); out.push(`<text x="${lx}" y="${l.mid + 15}" font-size="13" font-weight="700" fill="${LANE_LABEL}">${esc(lines[1])}</text>`); }
  });
  out.push(`<line x1="0" y1="${H - 0.5}" x2="${W}" y2="${H - 0.5}" stroke="${HAIR}"/>`);

  // title + legend
  out.push(`<text x="${M.left}" y="34" font-size="18" font-weight="700" fill="${INK}">${esc(title)}</text>`);
  out.push(`<text x="${M.left}" y="52" font-size="12" fill="${MUTED}">Signals live inside each entity; health rolls up to the workload root.</text>`);
  out.push(legend(W, contentW, M));

  // edges — portal-style orthogonal connectors: one vertical trunk from the parent, a shared
  // horizontal bus, and vertical stubs up to each child. Rounded corners, no arrowheads.
  // Stubs are colored by the child's state so a degraded contribution is traceable; the trunk and
  // bus take the parent's rolled-up state. Dashed propagation edges are drawn as individual elbows.
  // edges. Unlabeled solid edges bundle by parent (clean tight look). Unlabeled dashed edges are
  // individual elbows. LABELED edges (solid or dashed) are routed individually with their pill dropped
  // on the edge's own horizontal level (unique per overlap group), so pills never overlap and each sits
  // on its own line.
  const solidByParent = new Map();
  const dashedPlain = [];
  for (const e of g.edges) {
    if (e.label) continue; // labeled handled separately below
  ...
  for (const [parentId, es] of solidByParent) {
    ...
    const kids = es.map((e) => ({ e, cb: boxOf(e.from), st: STATE[g.nodes.get(e.from).state] || STATE.unknown, rank: stateRank[g.nodes.get(e.from).state] })).sort((a, b) => a.cb.cx - b.cb.cx);
  ...
  for (const e of dashedPlain) {
    const cb = boxOf(e.from), pb = boxOf(e.to), st = STATE[g.nodes.get(e.from).state] || STATE.unknown;
  ...
  // labeled edges: individual routing, horizontal at this edge's reserved level, pill on that horizontal
  const labelPills = [];
  for (const [ln, es] of labByPLane) {
    ...
    for (const e of es.sort((a, b) => stateRank[g.nodes.get(a.from).state] - stateRank[g.nodes.get(b.from).state])) {
      ...
      // anchor the pill at the child's vertical drop (child x), so each pill clearly belongs to one line
      labelPills.push({ label: e.label, x: cb.cx, y: labelY });
    }
  }
  // draw all pills last so they sit on top of every line
  for (const lp of labelPills) out.push(edgeLabelPill(lp.label, lp.x, lp.y));
  out.push(`</g>`);

  // cards
  for (const id of g.nodes.keys()) out.push(entityCard(g.nodes.get(id), boxOf(id), HEADER_H, ROW_H));
  out.push(`</svg>`);
  return { svg: out.join("\n"), W, H };
}

// entity card: header (icon + name + state pill), and if it owns signals an attached 4-column table:
// [status symbol] name  [metric icon]  result
function entityCard(n, b, HEADER_H, ROW_H) {
  ...
  // header row — every element centered on one horizontal centerline (headerMid)
  const headerMid = sigs.length ? b.y + HEADER_H / 2 : b.y + b.h / 2;
  const iconScale = 0.82;
  p.push(`<g transform="translate(${b.x + 6},${(headerMid - 12 * iconScale).toFixed(2)}) scale(${iconScale})">${icon(pickIcon(name), MUTED)}</g>`);
  // state pill (top-right) — measure first so the name can be fit to the space before it
  ...
  if (qualifier) {
    p.push(`<text x="${nameX}" y="${(headerMid - 5 + nsize * 0.34).toFixed(2)}" font-size="${nsize}" font-weight="600" fill="${INK}">${esc(ntext)}</text>`);
    p.push(`<text x="${nameX}" y="${(headerMid + 7 + 9 * 0.34).toFixed(2)}" font-size="9" fill="${MUTED}">${esc(qualifier.replace(/[()]/g, ""))}</text>`);
  } else {
    p.push(`<text x="${nameX}" y="${(headerMid + nsize * 0.34).toFixed(2)}" font-size="${nsize}" font-weight="600" fill="${INK}">${esc(ntext)}</text>`);
  }
  const py = headerMid - 9;
  p.push(`<rect x="${px}" y="${py.toFixed(2)}" width="${pillW}" height="18" rx="9" fill="#ffffff" stroke="${st.border}" stroke-width="1"/>`);
  p.push(statusDot(n.state, px + 11, headerMid));
  p.push(`<text x="${px + 20}" y="${(headerMid + 10.5 * 0.34).toFixed(2)}" font-size="10.5" fill="${LANE_LABEL}">${STATE_LABEL[n.state] || "Unknown"}</text>`);

  if (sigs.length) {
    const tY = b.y + HEADER_H + 3;
    p.push(`<line x1="${b.x + 1}" y1="${tY}" x2="${b.x + b.w - 1}" y2="${tY}" stroke="${HAIR}"/>`);
    const iconX = b.x + 32, nameX = b.x + 52, resX = b.x + b.w - 12;
    sigs.forEach((s, i) => {
      const ry = tY + 6 + i * ROW_H + ROW_H / 2;
      if (i > 0) p.push(`<line x1="${b.x + 10}" y1="${ry - ROW_H / 2}" x2="${b.x + b.w - 10}" y2="${ry - ROW_H / 2}" stroke="${HAIR}" stroke-opacity="0.7"/>`);
      p.push(statusDot(s.state, b.x + 16, ry));
      p.push(metricIcon(iconX, ry - 7, 14));
      const maxName = 26, nm = s.name.length > maxName ? s.name.slice(0, maxName - 1) + "…" : s.name;
      p.push(`<text x="${nameX}" y="${ry + 3.5}" font-size="10.5" fill="${INK}">${esc(nm)}</text>`);
      const rs = STATE[s.state] || STATE.healthy;
      p.push(`<text x="${resX}" y="${ry + 3.5}" font-size="10.5" font-weight="${s.state === "healthy" ? 400 : 600}" fill="${s.state === "healthy" ? MUTED : rs.dot}" text-anchor="end">${esc(s.result ?? "")}</text>`);
    });
  }
  return p.join("");
}
function legend(W, contentW, M) {
  const items = [["Healthy", STATE.healthy.dot], ["Degraded", STATE.degraded.dot], ["Unhealthy", STATE.unhealthy.dot], ["Unknown", STATE.unknown.dot]];
  // measure then right-align so nothing clips
  let total = 0;
  for (const [label] of items) total += 26 + label.length * 6.4;
  total += 22 + "Metric".length * 6.4;
  let x = W - 40 - total, yy = 44;
  const p = [`<text x="${x - 12}" y="${yy + 4}" font-size="11.5" font-weight="600" fill="${MUTED}" text-anchor="end">Legend</text>`];
  for (const [label, color] of items) { p.push(`<circle cx="${x + 6}" cy="${yy}" r="4.5" fill="${color}"/>`); p.push(`<text x="${x + 16}" y="${yy + 4}" font-size="11.5" fill="${LANE_LABEL}">${label}</text>`); x += 26 + label.length * 6.4; }
  p.push(metricIcon(x, yy - 7, 14)); p.push(`<text x="${x + 18}" y="${yy + 4}" font-size="11.5" fill="${LANE_LABEL}">Metric</text>`);
  return p.join("");
}
...
for (const blk of blocks) {
  try {
    const g = parseGraph(blk.code);
    if (g.nodes.size === 0) throw new Error("no nodes parsed");
    foldSignals(g);
    const lay = layout(g);
    const { svg, W, H } = render(g, lay, titleFor(blk.heading));
    if (!svg.includes("</svg>")) throw new Error("incomplete svg");
    writeFileSync(`${outDir}/${blk.slug}.svg`, svg, "utf8");
    manifest.push({ slug: blk.slug, svg: blk.slug + ".svg", nodes: g.nodes.size, lanes: lay.L, w: W, h: H });
    ok++; console.log(`  ok  ${blk.slug}.svg  (${g.nodes.size} nodes, ${lay.L} lanes, ${W}x${H})`);
