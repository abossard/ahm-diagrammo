...
**Rendered viewers** (GitHub's file view, VS Code's Markdown preview) see a real SVG `<img>` for
the diagram, with the Mermaid source tucked into a collapsed, still-editable `<details>` right
below it. **Raw editors** (opening this file to make a change) see exactly what they'd expect: a
plain ` ```mermaid ` fence, just wrapped in a machine-owned managed block that stays in sync.
